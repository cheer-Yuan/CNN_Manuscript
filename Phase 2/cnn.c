...
float cnntest(CNN* cnn, ImgArr inputData,LabelArr outputData,int testNum)
{
    ...
    for(n=0;n<testNum;n++){
        ...
        if(vecmaxIndex(cnn->O3->y,cnn->O3->outputNum)!=vecmaxIndex(outputData->LabelPtr[n].LabelData,cnn->O3->outputNum))
            incorrectnum++;
        cnnclear(cnn);
    }
...
void cnnbp(CNN* cnn,float* outputData) // backwards propaganda
{

    int i,j,c,r;    // note down the error
    for(i = 0; i < cnn->O3->outputNum; i++)     //i < 10
        cnn->e[i] = cnn->O3->y[i] - outputData[i];
